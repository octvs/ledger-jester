...
class OfxConverter(Converter):
    ...
    def format_balance(self, statement):
        ...
        elif hasattr(statement, "end_date") and hasattr(
            statement.end_date, "strftime"
        ):
    ...
    def format_initial_balance(self, statement):
        if hasattr(statement, "balance"):
            ...
            return Transaction(
                ...
                postings=[
                    posting,
                    posting.clone_inverted("Assets:Equity").format(
                        self.indent
                    ),
                ],
                date_format=self.date_format,
    ...
    def _finalize_postings(
        self, acct1, acct2, txn, security, posting_metadata
    ):
        ...
        posting_list.append(
            Posting(
                ...
                unit_price=Amount(
                    txn.unit_price, self.currency, unlimited=True
                ),
                metadata=posting_metadata,
            )
        )
        posting_list.append(
            Posting(
                acct2,
                Amount(
                    txn.units * txn.unit_price + txn.fees,
                    self.currency,
                    reverse=True,
                ),
            )
        ...
        if txn.commission:
            posting_list.append(
                Posting(
                    "Expenses:Commission",
                    Amount(txn.commission, self.currency),
                )
            )
...
class PaypalAlternateConverter(CsvConverter):
    ...
    def convert(self, row):
        if (
            ...
            posting_metadata = {"csvid": self.get_csv_id(row)}
            posting = Posting(
                self.name, self.mk_amount(row), metadata=posting_metadata
            )
            if (
                row["Type"] == "Add Funds from a Bank Account"
                or row["Type"] == "Charge From Debit Card"
...
class AmazonConverter(CsvConverter):
    ...
    def mk_amount(self, row, reverse=False):
        ...
        if currency == "USD":
            currency = "$"
        return Amount(
            Decimal(re.sub(r"\$", "", row["Item Total"])),
            currency,
            reverse=reverse,
        )
...
class MintConverter(CsvConverter):
    ...
    def convert(self, row):
        ...
        if row["Transaction Type"] == "credit":
            ...
            posting = Posting(
                account, self.mk_amount(row), metadata=posting_metadata
            )
            postings = [
                posting,
